...
- 用 **RLCD** 训练，即**面向校准决策的强化学习**。它替代 RLHF 的「人类偏好」目标，改让报出的概率与真实结果对得上。人类偏好不等于事实正确，RLHF 把模型往「让人满意」的方向推，代价可能是它依据事实做判断的能力被削弱。
- 命名取自杰文斯悖论——单次决策变便宜之后，调用频率反而越来越高。

## 2. 怎么用

一次调用由两部分组成：一个 state，加一组 questions。

**state** 是全部问题共享的上下文，可以是字符串、JSON 对象或文本数组。只放问题真正需要的信息，多余内容会拉低准确率。

**questions** 里每个问题都要先声明答案的形状，一共三种原语：
...
问题之间并行且相互独立地评估，所以加问题几乎不增加响应时间，也不会互相干扰。

它只负责判断：TypeSafe 的指导是把控制流留在你的代码里，让 Jev 做**原子化、结构化的决策**，而不是一次性做庞大的判断。
...
- **批量分类**。state 固定，一个请求里问许多狭窄的问题，然后读概率。这是最可能在生产环境里站住的模式。

这些环节可以写成正则和硬编码，需要的只是一个便宜、够快、能被代码直接调用的判断层。TypeSafe 自己的说法是 **smart if-statement**。

### Jev 不适合哪些任务？

- **System Two 任务**。需要深度推理的场景它做不了。
- **不是 chat 模型**。它不生成文本，没法对话；得先把问题写成答案形状固定的判断，官方形容为「像出多选题」。
...
官方给的整体口径是快 20 到 200 倍、便宜 40 到 400 倍。差额来自另外三件事：

- **输出免费**。定价只按输入 token 计，输出不计费。生成模型每次判断都要输出文本，而输出定价通常是输入的数倍。
- **延迟**。官方公布的端到端响应为 70ms 到 500ms。这个量级才允许同步调用，否则只能进异步队列。
